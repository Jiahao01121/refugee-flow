...
export default class RefugeeRoute_textArea_content_ibcCountry extends React.Component {
  ...
  cardGenerator(){
    ...
    cardItemAll = _.sortBy(cardItemAll,d => d.props.data['totalCross']).reverse();
  ...
  handlePageClick(param,pageMax){
    ...
    if(param === 'down'){
      ...
      if(this.state.currentPage < pageMax){
        d3.select('#CardContainer').transition().delay(200).style('opacity',0).on('end',()=>{
      ...
      d3.select('#CardContainer').transition().delay(400).style('opacity',1);
    ...
    return(
      <Wrapper className='route-map-titleGroup__IBC'>
        ...
        <DataSource onClick={() => window.open('https://frontex.europa.eu/', '_blank')}>
          ...
          <SearchBarInput inputOn = {this.state.inputOn} ></SearchBarInput>
          ...
          <PageDown
            onClick = {() => this.handlePageClick('down',pageMax)}>{">"}</PageDown>
      </Wrapper>
    )
  }
}
